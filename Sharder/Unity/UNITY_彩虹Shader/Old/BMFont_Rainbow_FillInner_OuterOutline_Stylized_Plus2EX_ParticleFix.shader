Shader "Custom/BMFont_Rainbow_FillInner_OuterOutline_Stylized_Plus2EX_iOS_SyncCoord"
{
    Properties
    {
        // === 基本設定 ===
        ...
        _SweepGroupMode   ("掃光群組", Range(0,1)) = 0
        _SweepFlowMode    ("掃光流動模式", Range(0,3)) = 0
        _SweepAngle       ("掃光角度", Range(0,360)) = 20
        _SweepSpeed       ("掃光速度", Range(-5,5)) = 1.0
        _SweepPixelsPerCycle ("每循環像素數", Range(8,4096)) = 240                
        _SweepWidthPx        ("掃光帶寬度", Range(0, 512)) = 40                         
        _SweepFeatherPx      ("羽化", Range(0, 64)) = 4
        
        _SweepRefHeight      ("畫面參考高度", Float) = 1080
        _SweepUseObject    ("線性模式使用物件座標", Float) = 1
        _SweepObjScaleComp ("線性縮放補償", Range(0,1)) = 1
        _SweepObjTiling    ("平鋪", Range(0.0001, 20)) = 0.2          
        _SweepObjWidth     ("掃光帶寬度", Range(0, 20)) = 0.08   
        _SweepObjFeather   ("羽化", Range(0, 100)) = 0.01   
        
        _RadialCenter      ("放射中心", Vector) = (0.5,0.5,0,0)
        _RadialTurns       ("放射每圈旋轉數", Range(0.1,10)) = 1.0
        _SpiralScale       ("螺旋半徑比例", Range(-10,10)) = 1.0
        _RadialBands       ("放射色帶數", Range(0.1,20)) = 3.0
        _FlowRefHeight     ("流動參考高度", Float) = 1080

        [Toggle(_SWEEP_MASK_ON)] _SweepMaskEnable   ("啟用掃光遮罩", Float) = 0
        _SweepMaskTex      ("掃光遮罩 (R)", 2D) = "white" {}
        _SweepMaskFeatherPx("掃光遮罩羽化", Range(0,64)) = 2
        _SweepMaskInvert   ("掃光遮罩反相", Float) = 0

        _SweepUseGradient  ("使用漸層貼圖", Float) = 0
        _SweepGradient     ("漸層貼圖 (RGB)", 2D) = "white" {}
        
        _SweepColor        ("掃光顏色", Color) = (1,1,1,1)
        _SweepColorMode    ("混合模式", Range(0,3)) = 1
        _SweepColorIntensity("顏色強度", Range(0,3)) = 1
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                #ifdef _SWEEP_ON
                {
                    ...
                    if (_SweepFlowMode < 0.5) // Linear
                    {
                        ...
                        {
                            // C# 腳本同步支援
                            bool useForcedUV = (i.uvMinSize.z > 50.0);
                            ...
                            if (useForcedUV) {
                                ...
                                // [Fallback Mode]
                                bool hasUV2 = (i.uvMinSize.z > 1e-6 && i.uvMinSize.w > 1e-6);
                                if (hasUV2) {
                                    ...
                                    if (_SweepUseObject > 0.5) band = sweepBand_object_linear(i.wxy, i.objScaleXY, _SweepAngle, _SweepObjTiling, _SweepObjWidth, _SweepObjFeather, tSweep, _SweepObjScaleComp);
                                    else band = sweepBand_screen_linear(posPx, _SweepAngle, _SweepPixelsPerCycle, _SweepWidthPx, _SweepFeatherPx, tSweep);
                                }
                            }
                        }
                    }
                    ...
                    if (_SweepExcludeFill > 0.5) band *= (1.0 - fillMaskTexVal);
