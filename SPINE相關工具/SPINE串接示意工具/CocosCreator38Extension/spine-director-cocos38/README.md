...
- 先停在要設定的時間，再於 Scene／Inspector 移動物件，只在該時間新增 Key。
- 同一時間反覆調整會更新同一個 Key，不會累積重複 Key。
- 拖曳時間、播放預覽及停止還原不會產生 Key；預覽位置不再被誤認為手動位移。
- 切換時間或關閉錄製時，舊的非同步位置取樣會被捨棄，避免寫入錯誤時間。
- 不會自動刪除舊版已產生的多餘 Key，請確認後手動刪除，以保留既有作品。
